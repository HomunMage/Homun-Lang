...
> Compiles to Rust. No `def`, no classes, no interfaces, no methods, no `impl`. All functions are `\() -> Type {}`. Structs are pure data. Braces `{}`, `:=` assignment, `==` equality, 1-based indexing, Python keywords. UFCS `.` for composition.
...
**No `def`, no `func`, no `fn`, no `return`.** Last expression is the return value. Functions use the `\` prefix to eliminate parsing ambiguity.
...
Parsing rules:
- `\(...)` → always a function (unambiguous, LL(1) parsable)
- `\(...)` followed by `->` → function with explicit return type
- `\(...)` followed by `{` → function with inferred return type
- `(...)` without `\` → grouped expression or tuple (never a function)
...
|---|---|---|
| `\(...)` | `\` | This is a function/lambda, not a tuple or grouped expression |
| `@{...}` | `@{` | This is a set literal, not a block or dict |
