...
All strings support interpolation by default (no `f` prefix needed). `{...}` marks interpolated expressions — inside a string, `{` always starts interpolation.

Escaping: `\\` → literal `\`, `\n`, `\t`, `\"` — standard escape sequences. `\{` → literal `{`, `\}` → literal `}` (to suppress interpolation).
...
* `"...{expr}..."` — `{` inside string → start of interpolation (escape with `\{` for literal)
