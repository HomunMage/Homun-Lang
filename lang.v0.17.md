...
- `use` imports from Rust libraries exposed to the scripting layer — like pyo3 for Python.
...
- All collections share the `@` prefix — lists `@[]`, dicts `@{}`, sets `@()` — with optional `<T>` type parameters: `@<int>[]`, `@<str, int>{}`, `@<str>()`.
...
- No mutual recursion — data flows forward, never in circles. Self-recursion is allowed; cross-lambda cycles are a compile error.
- No try/catch, no exceptions — the Rust engine owns error handling. Scripts fail safely, like Unity C# scripts.
- `break => value` for early return — no `return` keyword, extends the familiar `break` concept.
- 0-based indexing throughout, matching Rust and the engine stack.
...
```
dfs := (graph: @<int, @<int>[]>{}, start: int) -> @<int>[] {
  visited := @<int>[]
  stack   := @[start]
  while (len(stack) > 0) do {
    node  := stack[-1]
    stack := stack[:-1]
    if not node in visited do {
      visited := visited + @[node]
      for neighbor in graph[node] do {
        if not neighbor in visited do {
...
| Minimal syntax noise | No `;`, no `mut`, no bare `=` |
...
| Rust-aligned indexing | 0-based, Python-style `[start:end)` slicing |
| Game data pipeline | Native RON support — data structs are format and code simultaneously |
| Transparent recursion | Compiler auto-detects self-recursive lambdas — no `rec` keyword needed. Mutual recursion is forbidden (flux design). |
| Unambiguous void return | `-> _` means this lambda produces nothing — `_` is the discard marker, consistent with destructuring |
| Early return without `return` | `break => value` exits the enclosing lambda — extends the familiar `break` concept |
| Engine-owned error handling | No try/catch — the Rust engine catches script failures and keeps the game running, like Unity |
...
Homun is **strongly typed** — every value has a known type at compile time. The compiler infers
types from context when possible. When it can't, you annotate with `<T>`.

**If the compiler can figure it out, you don't write it. If it can't, you do.**

```
items := @[1, 2, 3]        // @<int>[] — inferred from contents
scores := @{"a": 10}       // @<str, int>{} — inferred
empty := @<int>[]           // empty — must annotate, no contents to infer from
grid  := @<@<int>[]>[]     // nested types work naturally
```

| Literal | Meaning | Rust type |
|---|---|---|
| `@[1, 2, 3]` | List of int (inferred) | `Vec<i64>` |
| `@<int>[]` | Empty list of int (explicit) | `Vec<i64>` |
| `@{"a": 1}` | Dict (inferred) | `HashMap<String, i64>` |
| `@<str, int>{}` | Empty dict (explicit) | `HashMap<String, i64>` |
| `@("x", "y")` | Set (inferred) | `HashSet<String>` |
| `@<str>()` | Empty set (explicit) | `HashSet<String>` |

The `@<T>` syntax works identically as a **type annotation** in lambda parameters, struct fields,
and return types. See the Lambdas section for typed parameter syntax.
...
Lambdas can declare type parameters with `<T>` before the parameter list. This creates a
**polymorphic function** — it works on any type that satisfies the usage within the body.
...
map_pair := <T, U>(pair: (T, T), f: (T) -> U) -> (U, U) {
  f(pair.0), f(pair.1)
}
```

Generic lambdas are always compiled to **Rust named generic functions** via the two-stage compiler
(same as recursive lambdas). The compiler detects `<T>` in stage one and emits a `fn<T>(...)`
...
Generic and recursive can combine:

```
// generic + recursive — compiler emits a named generic fn
flatten := <T>(nested: @<@<T>[]>[]) -> @<T>[] {
  if (len(nested) == 0) do { break => @<T>[] }
...
|---|---|
| Non-recursive, no `<T>` | Rust closure `\|...\| { ... }` |
| Recursive (self-referencing) | Rust named `fn` |
| Generic (`<T>`) | Rust named `fn<T>(...)` |
| Generic + recursive | Rust named `fn<T>(...)` |
...
names := @["Alice", "Bob", "Charlie"]  // @<str>[]
empty := @<int>[]                  // empty list requires type annotation
```
...
```
scores    := @{"alice": 100, "bob": 80}   // @<str, int>{}
empty_map := @<str, int>{}                 // empty dict requires type annotation
```
...
```
visited   := @(1, 3, 5, 7)            // @<int>()
flags     := @("fire", "ice", "poison")  // @<str>()
empty_set := @<int>()                  // empty set requires type annotation
```
...
`range` is usable in `for` loops, comprehensions, and anywhere a sequence is expected:
...
mid    := @[x for x in range(3, 7)]
evens  := @[x for x in range(2, 20, 2)]
```
...
| Generic lambda `<T>(x: T)` | Rust named `fn<T>(x: T)` (two-stage compile) |
| `@[...]` / `@<T>[...]` list | `Vec<T>` |
...
identity := <T>(x: T) -> T { x }                  // generic lambda
first    := <T>(items: @<T>[]) -> T { items[0] }   // generic with collection
...
// Collections (all use @ prefix, <T> when needed)
items  := @[1, 2, 3]                  // @<int>[] inferred
scores := @{"alice": 100, "bob": 80}  // @<str, int>{} inferred
flags  := @("fire", "ice", "poison")  // @<str>() inferred
empty  := @<int>[]                     // empty requires <T>
