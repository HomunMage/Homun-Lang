# Homun Script Lang Specification

> Target: Compiles Homun lang to Rust as ECS game engine script for client developer.
> No `def`, no classes, no interfaces, no methods, no `impl`. All functions are `*params* -> Type { body }` (where `*` represents `|` delimiters). Structs are pure data. Braces `{}` for blocks only, `:=` assignment, `==` equality, 1-based indexing, Python keywords. Pipe use `.`. `@` is the collection prefix — `@[list]`, `@{dict}`, `@(set)`, comprehensions. String interpolation uses `${}`.
>
> **Design decisions (intentional, not negotiable):**
> - `:=` is the **only** assignment operator — binding, reassignment, dict update, all use `:=`. This is a scripting language for game client developers, not a systems language. One operator, zero confusion. The compiler resolves binding vs reassignment via scope analysis. Don't fucking tell me about mutation issue.
> - `.` is the **pipe operator** doubling as field access — disambiguated by `(`. This is the core ergonomic choice: `x.f(args)` pipes, `x.name` accesses. Familiar dot-call syntax without methods or `impl`.
> - **1-based indexing** is intentional — this language targets game designers and client developers, not systems programmers. Math-natural indexing. The compiler handles the `-1` translation to Rust; users never think about it.
> - **`@` is the collection prefix** — `[` is ONLY for indexing, `{` is ONLY for blocks. All collection construction uses `@`. Zero ambiguity.
...
data["x"] := 3                 # new key (indexing uses [])
data["x"] := 5                 # update key
```

- `:=` for everything: binding, reassignment, collection index updates
...
- `Dict<K, V>` — hash map (literal: `@{"a": 1, "b": 2}`)
...
`[` after an expression is **always indexing** — never a list literal.
...
Slicing — `..` operator, inclusive both ends:
```
...
All strings support interpolation by default (no `f` prefix needed). `${...}` marks interpolated expressions — inside a string, `$` followed by `{` always starts interpolation. Bare `{` and `}` are plain text — no escaping needed.

Escaping: `\\` → literal `\`, `\n`, `\t`, `\"` — standard escape sequences. `\$` → literal `$` (to suppress interpolation before `{`).
...
**Rule 1: Struct and Enum names must be PascalCase.**
This includes enum variant names.

```
struct TreeNode { val: int }       # ✓
struct tree_node { val: int }      # ✗ compiler error

enum Shape { Circle, Rect }       # ✓
enum shape { circle, rect }       # ✗ compiler error
```

**Rule 2: Value bindings cannot start with uppercase.**
Variables, functions, lambdas — all values.

```
x := 5                            # ✓
Dog := 5                          # ✗ compiler error

add := |a, b| { a + b }          # ✓
Add := |a, b| { a + b }          # ✗ compiler error
```

Functions are values in this language (no `func` keyword — just lambdas bound to variables), so they follow the same rule as variables.

Summary:

| Kind | Rule | Example |
|------|------|---------|
| Struct name | must PascalCase | `TreeNode`, `Connection` |
| Enum type name | must PascalCase | `Shape`, `Result` |
| Enum variant | must PascalCase | `Circle`, `Rect`, `Empty` |
| Value (variable, function) | cannot start uppercase | `x`, `add`, `my_func` |

---

## B. Standard Library (`std`)
...
| `upto(start, end)` | inclusive range |
...
## C. Sugar Candy Syntax

> **Deferred** — convenience forms that desugar to A+B basics. Implement these after A and B are working.
...
Comprehensions use `@[...]`, `@{...}`, and `@(...)` — the **same prefix as collection literals**. The parser disambiguates: after parsing one expression, `for` → comprehension, `,` or close → literal.

```
# @[expr for ...] → List
squares := @[x * x for x in upto(5)]

# @{k: v for ...} → Dict
square_map := @{x: x * x for x in upto(5)}
...
Desugars to `upto.map(f).filter(g).to_list()` (or `.to_dict()`, `.to_set()`).
...
```
# sugar
for i in upto(5) { ... }          # 1, 2, 3, 4, 5

# desugars to
for i in upto(1, 5) { ... }       # inclusive both ends
```

Full form: `upto(start, end)` is **inclusive on both ends**.
```
for i in upto(1, 10) { ... }      # 1, 2, ..., 10
for i in upto(1, 10, 2) { ... }   # 1, 3, 5, 7, 9
for i in upto(10, 1, -1) { ... }  # 10, 9, ..., 1
```
...
| `x.map(f).filter(g)` | `homun_std::map(x, f)` etc. |
| `x.f().g()` | `g(f(x))` |
| `band` / `bor` / `bxor` | `&` / `\|` / `^` |
| `shl` / `shr` | `<<` / `>>` |
...
Function syntax (uses `*` notation since `|` conflicts with markdown tables):

* `*x* -> int { x + 1 }` → `|x| -> i64 { x + 1 }` (typed closure)
* `*a: int, b: int* -> int { a + b }` → `|a: i64, b: i64| -> i64 { a + b }`
* `** -> int { 42 }` → `|| -> i64 { 42 }` (no params)
...
| Token(s) | Meaning | Never means |
|---|---|---|
| `\|...\|` | function/lambda | bitwise OR (that's `bor`) |
| `@[` | list literal or list comprehension | — |
| `@{` | dict literal or dict comprehension | — |
| `@(` | set literal or set comprehension | — |
| `[` after expr | indexing / slicing | list literal (that's `@[`) |
| `{` | block (if/for/match/fn body) or struct literal | dict literal (that's `@{`) |
| `(` | grouping or tuple | function (that's `\|...\|`) |
| `${` inside string | interpolation start | — |
| `break =>` | break with value | — |

**`@` collection disambiguation:** After `@[`, `@{`, or `@(`, the parser parses one expression then checks the next token: `for` → comprehension, `,` or close bracket → literal. LL(1) — no backtracking needed.

**Struct literal vs block:** `PascalCase {` → struct literal (naming rules guarantee this). Lowercase or keyword before `{` → block.
