...
> Compiles to Rust. No `def`, no classes, no interfaces, no methods, no `impl`. All functions are `() -> Type {}`. Structs are pure data. Braces `{}`, `:=` assignment, `==` equality, 1-based indexing, Python keywords.
...
### Inline in chains (sugar shines here)
```
...
sorted := arr.sort(key=(x) { x[2] })       # sort by 3rd element
sorted := arr.sort(reverse=true)            # descending
sorted := arr.sort(key=(x) { -x })         # custom order
```
...
is_valid := (s: str) -> bool {
    ...
    for ch in s {
        if (ch in "([{") { stack := stack + [ch] }
            ...
            pairs := {")" : "(", "]" : "[", "}" : "{"}
            if (stack.len() == 0 or stack[-1] != pairs[ch]) { break false }
            stack := stack[:-1]
        }
    }
    stack.len() == 0
